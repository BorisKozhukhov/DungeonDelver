...
public class TileCamera : MonoBehaviour
{
    ...
    public void LoadMap()
    {
        // TILE_ANCHOR играет роль родителя для всех плиток Tile
    ...
    void ShowMap()
    {
        ...
        // Просмотреть всю карту и создать плитки где это необходимо
        for (int j=0; j<H; j++)
        {
            for (int i=0; i<W; i++)
            {
    ...
    // Перенруженная float версия GET_MAP
    ...
    static public void SET_MAP(int x, int y, int tNum)
    {
        ...
        if (x < 0 || x >= W || y < 0 || y >= H)
            return; // Предотвращение исключения IndexOutOfRangeException
        MAP[x, y] = tNum;
    }
}
